    ...
    this.props.form.validateFields((err, values) => {
      ...
      if (!err) {
        if(this.props.singleRowData){
          ...
          api.axiosPost('department_edit',editDate).then((res)=>{
              if(res.data.code ===1){
          ...
          api.axiosPost('department_add',addDate).then((res)=>{
              if(res.data.code ===1){
                  message.success(res.data.msg)
                  this.hideModal()
                  this.getList()
          ...
          <Form.Item label="部门名称">
              {getFieldDecorator('name', {
                rules: [
                  ...
                  { min:3,max:10,message: '部门名称长度范围3-10字符'}
                ],
              })(
                <Input placeholder="请输入部门名称" />,
              )}
          </Form.Item>
          <Form.Item label="职能描述" wrapperCol={{ span: 18 }}>
            {getFieldDecorator('info', {
              rules: [
                { max:100,message: '职能描述最多可输入100字符'}
              ],
            })(
